...
func TestImportRoundTrip(t *testing.T) {
	...
	dst := t.TempDir()
	runCmd(t, dst, "init")
	if _, err := runCmd(t, dst, "import", out); err != nil {
		t.Fatalf("import: %v", err)
	}
	b, err := os.ReadFile(filepath.Join(dst, ".ccs", "shared", "skills", "a.md"))
	if err != nil || string(b) != "A" {
		t.Errorf("imported content missing: %v / %q", err, b)
	}
}

func TestImportRefusesCrossPlatform(t *testing.T) {
	archivePath := filepath.Join(t.TempDir(), "cross.tar.gz")
	other := "linux"
	if importPlatformOverride == "linux" {
		other = "darwin"
	}
	...
	home := t.TempDir()
	runCmd(t, home, "init")
	out, err := runCmd(t, home, "import", archivePath)
	if err == nil {
